...
**Dependency upgrade release** — implements `DEPENDENCY_UPGRADE_PLAN.md` (the September 2026 review of every version-bearing surface). No behaviour of a working scaffold changes except where the plan found it broken.

### Fixed (broken for users today, independent of any version)

- **Retired model ids.** `claude-sonnet-4-20250514` (retired by Anthropic 2026-06-15) was hard-coded in seven agent templates and the CLI's LLM data generation; `gemini-2.0-flash` (shut down 2026-06-01) in the google-adk template; `gemini-1.5-flash` and Bedrock `claude-3-haiku` in memory examples. Every model literal now resolves through the new `create_context_graph.constants` module (`MODEL_DEFAULTS`, `RETIRED_MODEL_IDS`), generated projects read `settings.anthropic_model` / `openai_model` / `gemini_model` (env `ANTHROPIC_MODEL` / `OPENAI_MODEL` / `GEMINI_MODEL`), and `tests/test_model_defaults.py` fails if a retired id reappears anywhere in `src/` or `scripts/`.
- **google-adk returned "No response generated." on every ADK version** — the template read `event.text`, an attribute `google.adk.events.Event` never had. Rewritten: `async def` tools (ADK awaits them natively, so `nest-asyncio` and the `_run_sync` bridge are gone), text from `event.content.parts`, SSE streaming via `RunConfig(streaming_mode=StreamingMode.SSE)` with partial/aggregate de-duplication, errors logged instead of swallowed. google-adk no longer needs `--loop asyncio`.
- **crewai `/chat/stream` never ran the crew** — `Crew(stream=True).kickoff()` returns a lazy iterator that was never drained, so the object's `repr` was returned as the answer. The worker now drains the stream; tool-call chunks are filtered out of the text; CrewAI telemetry is disabled in the generated `.env`.
- **Conversation history never reached the agents** — `neo4j-agent-memory`'s `get_context()` returns `{session_id, context, has_context}` with no `messages`; the generated `memory.py` now fills `messages` from `short_term.get_conversation()` (dropping the just-stored user turn) so multi-turn context is real. PydanticAI switched to `Agent(instructions=...)`, which is re-sent when history is non-empty.
- **MCP server could not start** — `python -m neo4j_agent_memory.mcp.server` has no `--backend` flag and needs the never-installed `[mcp]` extra. `make mcp-server` and `claude_desktop_config.json` now use the console script `neo4j-agent-memory mcp serve --backend ...` with the `[cli,mcp]` extras (skipped on pydanticai, whose `mcp` 2.x conflicts with `fastmcp<3`) and `MEMORY_ENDPOINT`.
- **Frontend Docker image could not build** — `Dockerfile.frontend` copied `.next/standalone` and `public/` that were never produced; `next.config.ts` now sets `output: "standalone"` and the `public/` copy is gone.
- **Jira Cloud** — `Jira(...)` never passed `cloud=True`, so `jql()` hit `/rest/api/2/search` (removed by Atlassian, 410 Gone); `cloud` is now derived from the URL and the library floor is 4.0. **Notion** — pinned `notion_version="2022-06-28"` (notion-client ≥2.6 defaults to the 2025-09-03 data-source API, which broke the database pass). **PyGithub** — `Github(auth=Auth.Token(...))` (positional token is removed in v3). **Slack** — `RateLimitErrorRetryHandler` so 429s no longer silently drop channels.
- **CLI bolt ingest stored zero entities without `OPENAI_API_KEY`** — neo4j-agent-memory embeds before writing and defaulted to OpenAI. The CLI now disables the (unused) extraction pipeline, uses a local sentence-transformers embedder when installed, and otherwise writes entities without embeddings with a visible notice.
- **npm wrapper crashed on Node <20.19 / <22.7** (ESM `import` in a package without `"type": "module"`); fixed, `engines` is `>=22`, command probing works on Windows.
- **Production compose** passed no `ANTHROPIC_API_KEY` to langgraph/strands/crewai backends, no NAMS variables at all, and lacked `NEO4J_dbms_security_procedures_unrestricted`; the generated `README` framework sections for four frameworks rendered empty (hyphen/underscore key mismatch).
- `execute_cypher`'s `timeout` was sent as a Cypher parameter (`$timeout`) instead of a transaction timeout; it now uses `neo4j.Query(timeout=...)`. Drivers are closed on every error path (driver 6.x warns on unclosed resources).

### Changed — dependency policy

- **CLI (`pyproject.toml`)**: floors at tested versions with major caps (`click>=8.2,<9`, `rich>=14,<16`, `pydantic>=2.12,<3`, `neo4j>=6.1,<7`, `anthropic>=1.0,<2`, `openai>=2.0,<4`, `pytest>=9.0.3,<10`, `pydantic-settings>=2.14.2,<3`, connector and parser caps, `hatchling>=1.27,<2`); the `ingest` extra is `neo4j-agent-memory[nams]>=0.5.0,<0.6.0` (the `[spacy,gliner]` extras pulled torch + CUDA into the CLI for an extraction pipeline it never runs); `httpx2` replaces `httpx` in the dev extra; `[tool.uv] required-version`, `.python-version`, `asyncio_default_fixture_loop_scope`. **`uv.lock` refreshed** (151 → 100 packages): `neo4j-agent-memory` 0.0.5 → 0.5.0 (the old lock predated every API the CLI imports), anthropic 1.4.0, openai 3.8.0, neo4j 6.3.0, fastapi 0.141.1, starlette 1.6.0, pytest 9.1.1, pypdf 6.17.0, lxml 6.1.3, pdf-oxide 0.3.77 — clearing every advisory the review found in the old lock. Python 3.13 and 3.14 are supported (classifiers + CI matrix).
- **Generated backend (`pyproject.toml.j2`, `FRAMEWORK_DEPENDENCIES`)**: `requires-python ">=3.11, <3.15"` (crewai keeps `<3.14`); every dependency has a floor and a cap; `pydantic-ai-slim[anthropic]>=2.30,<3` replaces the `pydantic-ai` meta-package (76 fewer distributions and no `openai>=3` pin); `claude-agent-sdk` dropped (never imported; 190 MB bundled CLI); `nest-asyncio` dropped; `langchain>=1.4,<2` + `langchain-anthropic>=1.7,<2` with the template on `langchain.agents.create_agent` (`create_react_agent` is removed in LangGraph 2.0); `strands-agents[anthropic]>=1.53,<2.0`; `google-adk>=2.2,<3`; `openai-agents>=0.21,<0.23` with an explicit model, `max_turns`, and tracing off by default; `crewai[anthropic]>=1.15,<2`. Memory extras: `[nams]` always on NAMS; `[litellm]` (with `litellm>=1.84.0,<2`) only for frameworks without native OpenAI/Anthropic memory adapters — pydanticai and openai-agents omit it, which stops uv silently downgrading litellm to 1.83.0; bolt scaffolds get the PyTorch CPU index for Linux and the spaCy model as a locked URL dependency instead of `spacy download`. `Dockerfile.backend` uses `python:3.13-slim`, a pinned uv, and `--locked` when a lockfile is present.
- **Generated frontend**: Next 16.3, `eslint-config-next` 16 with a scaffolded `eslint.config.mjs` (`npm run lint` previously failed for lack of a config), ESLint 9 (eslint-config-next 16 still caps its plugins at ESLint 9), react-markdown 10, lucide-react 1.x, `@types/node` 24, TypeScript 5.9, Chakra 3.37, NVL 1.2; unused `neo4j-driver` removed; `node:24-alpine`.
- **Strands** builds the `Agent` per request with no printing callback (a module-global agent raised `ConcurrencyException` on overlapping chats and leaked `agent.messages` across sessions).
- **Neo4j image** `5.26.0` → `5.26.30` (LTS; ~20 CVE fixes); GDS projections rewritten as Cypher projections (native projections are slated for deprecation); tests accept any patch-pinned 5.x or calendar tag.
- **Docs site**: Docusaurus 3.10.2 + `@docusaurus/faster` (required by `future.v4`), MDX v1 compat kept on explicitly, framer-motion 13, TypeScript 5.9, Node 22+.
- **CI**: all actions on the Node 24 runtime (checkout v7, setup-uv v10.0.1 by SHA with a pinned uv, setup-python v7, setup-node v7 + Node 24, upload-artifact v7, download-artifact v8); Python matrix 3.11–3.14; new jobs: generated-frontend `tsc` + lint + `next build`, docs typecheck + build, a Neo4j 2026.x integration leg; lock audit; `scripts/check_template_versions.py` compares the Jinja-templated pins against PyPI/npm/Docker Hub (Dependabot cannot read them); `.github/dependabot.yml` for uv, GitHub Actions and the two npm manifests. The release workflow no longer installs a floating `npm@12`.
- Docs and prose: "Node.js 18+" → "Node.js 22+ (24 LTS recommended)", `neo4j:5` → `neo4j:5.26.30`, framework dependency tables, memory extras snippets, OPENAI_API_KEY/LangGraph claim.

### Tests

- New: `tests/test_model_defaults.py` (retired-id sweep, settings wiring, telemetry opt-out), `tests/test_dependency_policy.py` (floors + caps on every framework dep, memory-extras gating, bolt torch index / spaCy model, connector caps, GDS projections, loop-override policy). The slow generated-project matrix now installs all eight frameworks. Fast suite: 1,622 passing with the connectors extra installed.
...
- **Generated `memory.py`**: `connect_memory()` runs `_ensure_nams_ontology()` after the client connects — already active → no-op; catalog match by domain id → activate its latest version; unknown domain → **create** the ontology from the scaffold's new `backend/app/ontology_document.json` and activate it (the custom-domain path). Best-effort and logged: memory still works on `nams-default` if the ontology API is unavailable. Covers app startup and `make seed` (which connects through the same path).
- **CLI ingest** (`run_nams_ingest`) and the **scaffolded `import_data.py`** run the same ensure sequence as stage 0, pinned by the parity contract test (`get_active → list → get → activate`, keyword-called so the recorder can compare shapes).
- **`build_nams_ontology_document()`** (`ontology.py`) produces the server's `OntologyDocument` shape — `{domain, entity_types, relationships}`, excluding app-side sections (`agent_tools`, `system_prompt`, `document_templates`, …) — used by both the renderer (writes `ontology_document.json` into every scaffold) and the CLI ingest.
