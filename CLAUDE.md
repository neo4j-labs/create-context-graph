...
├── constants.py        # MODEL_DEFAULTS / RETIRED_MODEL_IDS — single source of truth for model ids
...
│   ├── ontology_document.json  # NAMS ontology doc (create-on-connect for custom domains)
...
```bash
pytest tests/ -v                    # Fast suite: 1,622 passing with the connectors extra installed (see `make test`)
...
pytest tests/test_cli.py            # CLI integration + 8 domain/framework combos + neo4j types + validation + auto-slug + --neo4j-database + --ontology-file + Linear + Google Workspace + Claude Code connectors (84)
...
pytest tests/test_model_defaults.py  # Retired model ids never reappear; settings wiring; crewai telemetry opt-out (12)
pytest tests/test_dependency_policy.py # Floors + caps, memory-extras gating, bolt torch index/spaCy model, GDS projections (28)
pytest tests/test_generated_project.py # Deep validation: Python/TS/Cypher syntax, memory, neo4j types, streaming, QA fixes, async bridging, thread safety, tool prompts, embeddings config, NEO4J_DATABASE + NAMS cypher + scenario-fallback pins (269)
...
pytest tests/test_routes_integration.py # Mounts the generated FastAPI app: NAMS/bolt route dispatch, /cypher on NAMS, 503 guards, /health memory surfacing (17)
pytest tests/test_generated_client_runtime.py # Executes rendered context_graph_client.py + memory.py against doubles: NAMS dispatch/coercion, database threading, store_message error state (26)
...
pytest tests/test_integration.py --integration  # Neo4j integration: schema DDL, fixture ingestion, agent tool queries, domain scoping, explicit-database threading (10)
```
...
| Target | Description |
|--------|-------------|
| `make test` | Fast unit tests (locked env, no external deps) |
| `make test-slow` | Full suite including matrix + perf + generated project + connector/functional tests (1,866 passing with connectors extra) |
...
| `make lock-upgrade` | `uv lock --upgrade` — refresh the lock (commit it whole) |
| `make check-template-versions` | Compare Jinja-templated pins against PyPI/npm/Docker Hub |
...
| LangGraph | `langgraph/` | `@tool` + `langchain.agents.create_agent()` | Full (`graph.astream_events()`) |
| CrewAI | `crewai/` | `Agent` + `Task` + `Crew` with `@tool`, `run_coroutine_threadsafe` bridging | Tools only |
| Strands | `strands/` | per-request `Agent` with `@tool`, Anthropic model, `run_coroutine_threadsafe` bridging | Full (`stream_async`) |
| Google ADK | `google_adk/` | `Agent` + async `FunctionTool`s, Gemini model (no nest_asyncio) | Full (`runner.run_async()` + `StreamingMode.SSE`) |
...
|-----|---------|-------------|
| **test** | All pushes + PRs | Fast unit tests on Python 3.11 and 3.12 (`fail-fast: false` so one version's failure doesn't cancel the other) |
| **lint** | All pushes + PRs | Ruff on `src/` and `tests/` (rule set pinned in `[tool.ruff.lint]`, ruff version pinned in the dev extra) |
| **full-suite** | All pushes + PRs | Full test suite on 3.12: `--slow --functional` — 176 domain × framework matrix, perf tests, generated-project venv suites, local-file vault functional tests |
| **smoke-test** | Push to `main` only | Neo4j integration tests + E2E: scaffold → install → start → chat for all 8 frameworks (needs repository secrets, so it can't run on fork PRs) |

**Every job installs from `uv.lock` via `uv sync --locked --extra ...` and runs commands with `uv run --no-sync`.** Never mix `uv pip install` with a bare `uv run` in CI: `uv run` does an implicit lockfile sync that downgrades locked base deps underneath freshly-installed latest extras (this skew broke CI when a new anyio required a newer typing_extensions than the lock pinned). If `--locked` fails, run `uv lock` locally and commit the refreshed lockfile — dependency changes in `pyproject.toml` must ship with a matching `uv.lock`. The Makefile test/lint targets use `uv run --extra dev ...` so local runs resolve from the lock the same way.
...
A separate release workflow (`release.yml`) triggers on version tags (`v*`).

## Dependency policy

`DEPENDENCY_UPGRADE_PLAN.md` is the reference. Three surfaces, three rules: the CLI's `pyproject.toml` has floors + major caps and `uv.lock` is the source of truth (`make lock-upgrade` monthly and on advisories; commit the whole lock); generated projects (`templates/backend/shared/pyproject.toml.j2`, `config.py:FRAMEWORK_DEPENDENCIES`, `templates/frontend/package.json.j2`) pin every dependency with a floor and a cap because users install them fresh and unlocked — `tests/test_dependency_policy.py` enforces it; model ids live only in `src/create_context_graph/constants.py`. Dependabot covers uv/actions/npm; `make check-template-versions` covers the Jinja manifests Dependabot cannot read. Upstream pins that constrain choices: strands-agents `[anthropic]` requires `anthropic<1`, crewai `[anthropic]` requires `anthropic~=0.73` and Python `<3.14`, litellm requires `openai<3` (hence no `[litellm]` on pydanticai/openai-agents), eslint-config-next 16 requires ESLint 9, fastmcp<3 requires mcp<2 (hence no MCP extras on pydanticai).

## What's Not Yet Implemented

- Generated projects do not ship a lockfile; `make install` creates `backend/uv.lock` and the Dockerfile uses `--locked` only when it is present
